...
TARBALLURL="https://github.com/CryptoFlowCoin/CryptoFlowCoin/releases/download/v1.2.1.1/CryptoFlow-1.2.1.1-ubuntu_daemon_16.04.tar.gz"
TARBALLNAME="CryptoFlow-1.2.1.1-ubuntu_daemon_16.04.tar.gz"
TRTTVERSION="1.2.1.1"
...
echo "
 ...
 |        the masternode under a user crfl.     |::
 |                                                  |::
...
echo "" && echo 'Added user "crfl"' && echo ""
sleep 1
...
# Install CryptoFlow daemon
#wget $TARBALLURL && unzip $TARBALLNAME -d $USERHOME/  && rm $TARBALLNAME
wget $TARBALLURL && tar -xvf $TARBALLNAME -C $USERHOME/  && rm $TARBALLNAME
cp $USERHOME/cryptoflowd /usr/local/bin
cp $USERHOME/cryptoflow-cli /usr/local/bin
cp $USERHOME/cryptoflow-tx /usr/local/bin
rm $USERHOME/cryptoflow*
chmod 755 /usr/local/bin/cryptoflow*

# Create .cryptoflow directory
mkdir $USERHOME/.cryptoflow
...
# Create cryptoflow.conf
touch $USERHOME/.cryptoflow/cryptoflow.conf
cat > $USERHOME/.cryptoflow/cryptoflow.conf << EOL
rpcuser=${RPCUSER}
...
maxconnections=256
rpcport=3334
masternodeaddr=${IP_ADDRESS}:3333
masternodeprivkey=${KEY}
masternode=1
EOL
chmod 0600 $USERHOME/.cryptoflow/cryptoflow.conf
chown -R $USER:$USER $USERHOME/.cryptoflow
...
echo "" && echo 'Added user "crfl1"' && echo ""
sleep 1
...
# Create .cryptoflow directory
mkdir $USERHOME1/.cryptoflow

sleep 6


# Create cryptoflow.conf (second node)
touch $USERHOME1/.cryptoflow/cryptoflow.conf
cat > $USERHOME1/.cryptoflow/cryptoflow.conf << EOL
rpcuser=${RPCUSER1}
...
masternode=1
EOL
chmod 0600 $USERHOME1/.cryptoflow/cryptoflow.conf
chown -R $USER1:$USER1 $USERHOME1/.cryptoflow
