...
# Install Trittium daemon1
wget $TARBALLURL && tar -xvf $TARBALLNAME -C $USERHOME1/  && rm $TARBALLNAME
cp $USERHOME1/trittiumd /usr/local/bin
cp $USERHOME1/trittium-cli /usr/local/bin
cp $USERHOME1/trittium-tx /usr/local/bin
rm $USERHOME1/trittium*
chmod 755 /usr/local/bin/trittium*
# Create .trittium2 directory
